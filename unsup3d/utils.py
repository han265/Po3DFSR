...
def save_images(out_fold, imgs, imgs_name, epoch, prefix='', suffix='', sep_folder=True, is_val=False, ext='.png'):
    #print(imgs_name)
    #print(len(imgs))
    if sep_folder:
        out_fold = os.path.join(out_fold, suffix)
    ...
    for i, img in enumerate(imgs):
        ...
        if is_val:
            cv2.imwrite(os.path.join(out_fold, '{}_'.format(str(epoch)) + imgs_name[i]), im_out)
        cv2.imwrite(os.path.join(out_fold, imgs_name[i]), im_out)
